...
class LeaveBalanceReport(models.Model):
    ...
    _description = 'Báo cáo sử dụng thời gian nghỉ'

    employee_id = fields.Many2one('hr.employee', string="Nhân viên")
    job_id = fields.Many2one('hr.job', string="Vị trí công việc", related='employee_id.job_id', readonly=True)
    department_id = fields.Many2one('hr.department', string="Phòng ban", related='employee_id.department_id', readonly=True, store=True)
    leave_type_id = fields.Many2one('hr.leave.type', string="Loại nghỉ phép")
    allocated_days = fields.Float(string="Ngày nghỉ được cấp")
    taken_days = fields.Float(string="Ngày đã sử dụng")
    balance_days = fields.Float(string="Ngày còn lại")
    utilization_rate = fields.Float(string="Tỷ lệ sử dụng", compute='_compute_utilization_rate')
